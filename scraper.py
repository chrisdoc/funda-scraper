...
if (
    ...
    print(
        f"ERROR: The following environment variables must be set: {ENV_FUNDA_DB}, {ENV_TELEGRAM_TOKEN}, {ENV_CHAT_ID} and {ENV_SEARCH_URL}",
        file=sys.stderr,
    )
    exit(1)
...
for listing in newListing:
    ...
    telegramBot.postMessage(
        MESSAGE_TEMPLATE
        % (
            "mentioned" if listing.hasElevator else "not mentioned",
            re.sub(r"[^0-9.]", "", listing.stories),
            listing.price.replace(".", "\\.").replace("k\\.k\\.", ""),
            listing.link,
        )
    )
